    ...
    for spec_i in range(n_spectra):
        ...
        for rank_i in range(ranks_per_spectrum + mumble_ranks):
            ...
            if not is_original:
                ...
                # the constructor fails validation -- mutating the dict in place after
                # construction bypasses that check, matching how mumble sets this flag
                # (a real bool, not a string) when include_original_psm=True.
...
def test_write_rescoring_tables(tmp_path):
    ...
    assert (tmp_path / "test.weights.tsv").is_file()

    # TSV, not parquet, and human-readable (has a header row with expected columns).
    psms_tsv = pd.read_csv(tmp_path / "test.psms.tsv", sep="\t")
